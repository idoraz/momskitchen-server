...
export let getAllRecipes = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).send(
            `Oops! Unexpected error has occured: ${error.message}`
        );
    }
};
...
export let getRecipes = async (req: Request, res: Response) => {
    try {
        ...
        const recipes = await db
            .collection('recipes')
            .find({ 'category.key': req.params.categoryKey })
            .toArray();
        if (recipes) {
            res.send(recipes);
        } else {
            res.status(500).send(
                `Failed to retrieve recipes from DB by category kye: ${req.params.categoryKey}`
            );
        }
    } catch (error) {
        res.status(500).send(
...
export let getObjectByKey = async (req: Request, res: Response) => {
    try {
        ...
        if (recipe) {
            ...
            res.status(500).send(
                `Couldn't find ${req.params.key} in collection: ${req.params.collection}`
            );
        }
    } catch (error) {
        res.status(500).send(
...
export let searchRecipe = async (req: Request, res: Response) => {
    try {
        ...
        const recipes = await db
            .collection('recipes')
            .find({ name: { $regex: `.*${req.params.token}*` } })
            .toArray();
        if (recipes) {
            res.send(recipes);
        } else {
